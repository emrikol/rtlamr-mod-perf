...
func main() {
	...
	unchangedInterval, err := parseUnchangedInterval(os.Getenv("COLLECT_INFLUXDB_UNCHANGED_INTERVAL"))
	if err != nil {
		log.Fatalf("invalid COLLECT_INFLUXDB_UNCHANGED_INTERVAL: %s\n", err)
	}
	coalescer := NewPointCoalescer(unchangedInterval)
	...
	for stdinBuf.Scan() {
		...
		msg.AddPoints(logMsg, func(t time.Time, tags map[string]string, fields map[string]interface{}) {
			if !coalescer.ShouldEmit(t, tags, fields) {
				return
			}
